 ...
 **/
_defineProperty(UserController, "forgotUserPassword", async (req, res) => {
  try {
    const requestBody = req.body;
    // console.log("REQUEST BODY::: ", requestBody);
    ...
    const {
      error,
      value
    } = _auth_validator.default.forgotUserPasswordSchema.validate(requestBody);
    if (error) {
      const response = new _response.default(false, 400, `${error.message}`);
      return res.status(response.code).json(response);
    }
    ...
    //  Send OTP to users mail.
    const emailResponse = await _send_email.default.sendForgotPasswordMail(value.email, userName, subject, otp);
    console.log("EMAIL RESPONSE::: ", emailResponse.response);
 ...
 **/
_defineProperty(UserController, "resetUserPassword", async (req, res) => {
  try {
    const requestBody = req.body;
    ...
    const {
      error,
      value
    } = _auth_validator.default.resetUserPasswordSchema.validate(requestBody);
    if (error) {
      const response = new _response.default(false, 400, `${error.message}`);
      return res.status(response.code).json(response);
    }
    ...
    if (updatedUser[0] === 0) {
      const response = new _response.default(false, 400, "Failed to reset password.");
      return res.status(response.code).json(response);
    }
    ...
    const response = new _response.default(true, 200, "Password updated successfully. Kindly login with your new password");
    return res.status(response.code).json(response);
  } catch (error) {
    console.log(`ERROR::: ${error}`);
    const response = new _response.default(false, 500, 'Server error, please try again later.');
 ...
 **/
_defineProperty(UserController, "uploadUserProfileImage", async (req, res) => {
  try {
    const {
      id
    } = req.params;
    ...
    if (updatedUser[0] === 0) {
      const response = new _response.default(false, 400, "Failed to update user profile image.");
      return res.status(response.code).json(response);
    }
    ...
    const response = new _response.default(true, 200, "Profile image updated successfully.", {
      user
    });
    return res.status(response.code).json(response);
  } catch (error) {
