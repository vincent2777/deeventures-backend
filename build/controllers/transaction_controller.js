...
_defineProperty(TransactionController, "uploadTransactionProof", async (req, res) => {
  try {
    ...
    if (!req.files || req.files.length === 0) {
      const response = new _response.default(false, 400, "No files uploaded.");
      return res.status(response.code).json(response);
    }
    ...
    // Save the first file URL or process all URLs as needed
    {
      where: {
        trnx_reference
      }
    ...
    const response = new _response.default(false, 500, "An error occurred while processing your request.");
    return res.status(response.code).json(response);
  }
});
