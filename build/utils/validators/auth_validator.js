...
_defineProperty(AuthValidator, "signUpUserSchema", _joi.default.object({
  ...
  password: _joi.default.string().pattern(new RegExp('^[a-zA-Z0-9!@#$%^&*()_+\\-=\\[\\]{};:\'",.<>\\/?|]{6,30}$')).required().error(new Error("Password must be at least 6 characters long and can include alphanumeric and special characters."))
}));
/**
 ...
 **/
_defineProperty(AuthValidator, "forgotUserPasswordSchema", _joi.default.object({
  email: _joi.default.string().required().email()
}));
/**
 * @function resetUserPasswordSchema
 **/
_defineProperty(AuthValidator, "resetUserPasswordSchema", _joi.default.object({
  otp: _joi.default.string().required(),
  password: _joi.default.string().pattern(new RegExp('^[a-zA-Z0-9!@#$%^&*()_+\\-=\\[\\]{};:\'",.<>\\/?|]{6,30}$')).required().error(new Error("Password must be at least 6 characters long and can include alphanumeric and special characters.")),
  confirm_password: _joi.default.string().valid(_joi.default.ref('password')).required().error(new Error("Confirm password must match the password."))
}));
/**
